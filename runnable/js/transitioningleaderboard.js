...
function changeDate(event) {
       // console.log(document.getElementsByName("newdate").value);
        var val = d3.select('input[name="newdate"]:checked').node().value;
      //  console.log("val = " + val);
        eval(val);
}
...
function leaderBoard(date) {
 d3.csv("csv/" + date + ".csv" , function(error, medalCounts) {
   ...
   // console.log("standings = " + JSON.stringify(standings));
    ...
    // Prepare for radio button transition version by adding function updateLeaderboard.
    // When this function is added, comment out the '.style("top"' attr of the Update
    // statement above. Do this after the flagged leaderboard example, in which it 
    // it was necessary to struggle with assigning the "top" attribute to each div.datarow. 
    // This is not an exercise for the student; this is for the instructor tp illustrate
    // to illustrate.
    updateLeaderboard();
       ...
       // Debugging function
       function examineRow(d, i)
       {
        var entries = d3.entries(d);
        console.log("examineRow " + i + ": rowdata = " + JSON.stringify(entries));
        return entries;
       }
       ...
       {           
            ...
            // Here is a useful technique to see all the data bound to all the rows. 
            var rowdata = d3.selectAll("div.datarow").data();
           // console.log("rowdata = " + JSON.stringify(rowdata));
                      
            // Now let's look at the data bound to the individual data rows.
            // Do this by providing data to a "dummy" selection.
            // The callback function examineRow will be called once for each div.datarow.
            d3.selectAll("div.datarow").selectAll("dummy")
                           // Can't log rowdata, because it will be the update selection 
                           // .data(function(d) { return d3.entries(d); } );
                           .data(function(d, i) { return examineRow(d, i + 1); } );
       ...
       function reportPlace(type, d)
       {
          console.log(type + d.Team + ", place = " + d.Place);
       }
    
    
       // Assign the "top" attr to each div.datarow based on Rank.
       // This function illustrates the important technique of retrieving
       // the bound data as a dataset and then re-binding it with an additional
       // attribute, in this case the "top" attribute.                            
       function updateLeaderboard() {
           // This is a GET of data. 
           var dataset = d3.selectAll("div.datarow").data();
           
           console.log("dataset = " + JSON.stringify(dataset));
           
           // Add a style attribute to each member of dataset.
           d3.selectAll("div.datarow")
               // .data(dataset, function(d) { return d.Team })
                .data(dataset)
                .transition()
                .duration(2000)
                .style("top", function(d, i) { reportRank("Enter ", d); return (divTops.datarow + (d.Rank * 60)) + "px";});
       }
    
      // Use d3 to enter radio buttons in a data driven way.
      // var form = d3.select("#dateselector").selectAll("label")
       var form = d3.select("#dateselector").selectAll("p")
        .data(["Feb7", "Feb8"]);
